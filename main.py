...
# 检查输入的URL是Swagger首页、API文档地址还是resource地址，并返回相应的类型标识。
def check(url):
    try:
...
# 根据文件扩展名判断是要保存为CSV还是TXT文件，但函数体为空。
...
# 目前是空的，可能用于处理source类型的URL。
def go_source(url):
    pass


# 处理API文档地址，构造请求并发送到各个API端点，收集响应数据。
            ...
            try:
                for method in paths[res_path]:  # get/post/字符串
                    ...
                    if method == 'post' or method == 'put':  # 这两种请求，参数如何构造在接口文档中没有，暂时不知道在哪找，所以随便发一个包
                        ...
                        if "'in': 'body'" in text:
                            if method == 'post':
                                req = requests.post(url=domain + basePath + path, data=json_payload, timeout=5,
                                                    verify=False)
                                hhh = [url, summary, path, method, domain + basePath + path, param_num, json_payload,
                                       req.status_code, req.text]
                            else:
                                req = requests.put(url=domain + basePath + path, data=json_payload, timeout=5,
                                                   verify=False)
                                hhh = [url, summary, path, method, domain + basePath + path, param_num, json_payload,
                                       req.status_code, req.text]
...
def go_html(urlq, q):
    ...
    url = soup.find('span', class_='url').get_text()
    go_docs(url, q)
